...
class AccessGroup extends Model
{
    ...
    public function filter($order_field, $order_ascdesc, $search, $search_column, $limit, $startLimit)
    {
        ...
        if ($search != '' && $search != NULL) {
            ...
            ->orWhere('access_group.name', 'LIKE', "%{$search}%")
            ->orWhere('access_group.description', 'LIKE', "%{$search}%")
        ...
        if ($search_column['name'] != '' && $search_column['name'] != NULL) {
            $sql->where('access_group.name', 'LIKE', "%{$search_column['name']}%");
        }
        if ($search_column['description'] != '' && $search_column['description'] != NULL) {
            $sql->where('access_group.description', 'LIKE', "%{$search_column['description']}%");
        }
