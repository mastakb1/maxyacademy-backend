...
class OrderConfirm extends Model
{
    ...
    {
        return $this->belongsTo('App\OrderCourse', 'id_order_course', 'id');
    }
    ...
    {
        $sql = OrderConfirm::select('order_confirm.*', 'user_create.name as user_create_name', 'user_update.name as user_update_name', 'user_verified.name as user_verified_name', 'm_payment_type.name as payment_type', 'm_bank.name as bank', 'order_course.order_number', 'member.name as member', 'sbank.name as sender_bank', 'm_bank_account.account_name')
        ->join('users as user_create', 'user_create.id', 'order_confirm.created_id')
        ->join('users as user_update', 'user_update.id', 'order_confirm.updated_id')
        ->leftJoin('users as user_verified', 'user_verified.id', 'order_confirm.verified_id')
        ->join('m_payment_type', 'm_payment_type.id', 'order_confirm.id_m_payment_type')
        ->join('order_course', 'order_course.id', 'order_confirm.id_order_course')
        ->join('member', 'member.id', 'order_course.id_member')
        ->join('m_bank_account', 'm_bank_account.id', 'order_confirm.id_m_bank_account')
        ->join('m_bank', 'm_bank.id', 'm_bank_account.id_m_bank')
        ->join('m_bank as sbank', 'sbank.id', 'order_confirm.sender_bank')
        ->orderBy($order_field, $order_ascdesc);

        if ($search != '' && $search != NULL) {
            $sql->where('order_confirm.id', 'LIKE', "%{$search}%")
            ->orWhere('order_course.order_number', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.order_confirm_number', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.date', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.sender_account_name', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.sender_account_number', 'LIKE', "%{$search}%")
            ->orWhere('sbank.name', 'LIKE', "%{$search}%")
            ->orWhere('m_bank_account.account_name', 'LIKE', "%{$search}%")
            ->orWhere('m_bank.name', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.amount', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.status', 'LIKE', "%{$search}%")
            ->orWhere('member.name', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.verified_at', 'LIKE', "%{$search}%")
            ->orWhere('order_confirm.verified_comment', 'LIKE', "%{$search}%")
        ...
        if ($search_column['id'] != '' && $search_column['id'] != NULL) {
            $sql->where('order_confirm.id', 'LIKE', "%{$search_column['id']}%");
        }
        ...
        if ($search_column['order_confirm_number'] != '' && $search_column['order_confirm_number'] != NULL) {
            $sql->where('order_confirm.order_confirm_number', 'LIKE', "%{$search_column['order_confirm_number']}%");
        }
        if ($search_column['date'] != '' && $search_column['date'] != NULL) {
            $sql->where('order_confirm.date', 'LIKE', "%{$search_column['date']}%");
        }
        if ($search_column['sender_account_name'] != '' && $search_column['sender_account_name'] != NULL) {
            $sql->where('order_confirm.date', 'LIKE', "%{$search_column['sender_account_name']}%");
        }
        if ($search_column['sender_account_number'] != '' && $search_column['sender_account_number'] != NULL) {
            $sql->where('order_confirm.sender_account_number', 'LIKE', "%{$search_column['sender_account_number']}%");
        }
        ...
        if ($search_column['amount'] != '' && $search_column['amount'] != NULL) {
            $sql->where('order_confirm.amount', 'LIKE', "%{$search_column['amount']}%");
        }
        ...
        if ($search_column['status'] != '' && $search_column['status'] != NULL) {
            $sql->where('order_confirm.status', 'LIKE', "%{$search_column['status']}%");
        }
        if ($search_column['verified_at'] != '' && $search_column['verified_at'] != NULL) {
            $sql->where('order_confirm.verified_at', 'LIKE', "%{$search_column['verified_at']}%");
        }
        ...
        if ($search_column['verified_comment'] != '' && $search_column['verified_comment'] != NULL) {
            $sql->where('order_confirm.verified_comment', 'LIKE', "%{$search_column['verified_comment']}%");
        }
        if ($search_column['created_at'] != '' && $search_column['created_at'] != NULL) {
            $sql->where('order_confirm.created_at', 'LIKE', "%{$search_column['created_at']}%");
        }
        ...
        if ($search_column['updated_at'] != '' && $search_column['updated_at'] != NULL) {
            $sql->where('order_confirm.updated_at', 'LIKE', "%{$search_column['updated_at']}%");
        }
