...
class MPage extends Model
{
    ...
    {
        $sql = MPage::select('m_page.*', 'user_create.name as user_create_name', 'user_update.name as user_update_name')
        ->join('users as user_create', 'user_create.id', 'm_page.created_id')
        ->join('users as user_update', 'user_update.id', 'm_page.updated_id')
        ->orderBy($order_field, $order_ascdesc);

        if ($search != '' && $search != NULL) {
            $sql->where('m_page.id', 'LIKE', "%{$search}%")
            ->orWhere('m_page.name', 'LIKE', "%{$search}%")
            ->orWhere('m_page.type', 'LIKE', "%{$search}%")
            ->orWhere('m_page.heading', 'LIKE', "%{$search}%")
            ->orWhere('m_page.title', 'LIKE', "%{$search}%")
            ->orWhere('m_page.slug', 'LIKE', "%{$search}%")
            ->orWhere('m_page.meta_keyword', 'LIKE', "%{$search}%")
            ->orWhere('m_page.meta_description', 'LIKE', "%{$search}%")
            ->orWhere('m_page.status', 'LIKE', "%{$search}%")
            ->orWhere('m_page.created_at', 'LIKE', "%{$search}%")
            ->orWhere('user_create.name', 'LIKE', "%{$search}%")
            ->orWhere('m_page.updated_at', 'LIKE', "%{$search}%")
            ->orWhere('user_update.name', 'LIKE', "%{$search}%");
        }

        if ($search_column['id'] != '' && $search_column['id'] != NULL) {
            $sql->where('m_page.id', 'LIKE', "%{$search_column['id']}%");
        }
        if ($search_column['name'] != '' && $search_column['name'] != NULL) {
            $sql->where('m_page.name', 'LIKE', "%{$search_column['name']}%");
        }
        if ($search_column['type'] != '' && $search_column['type'] != NULL) {
            $sql->where('m_page.type', 'LIKE', "%{$search_column['type']}%");
        }
        if ($search_column['heading'] != '' && $search_column['heading'] != NULL) {
            $sql->where('m_page.heading', 'LIKE', "%{$search_column['heading']}%");
        }
        if ($search_column['title'] != '' && $search_column['title'] != NULL) {
            $sql->where('m_page.title', 'LIKE', "%{$search_column['title']}%");
        }
        if ($search_column['slug'] != '' && $search_column['slug'] != NULL) {
            $sql->where('m_page.slug', 'LIKE', "%{$search_column['slug']}%");
        }
        if ($search_column['meta_keyword'] != '' && $search_column['meta_keyword'] != NULL) {
            $sql->where('m_page.meta_keyword', 'LIKE', "%{$search_column['meta_keyword']}%");
        }
        if ($search_column['meta_description'] != '' && $search_column['meta_description'] != NULL) {
            $sql->where('m_page.meta_description', 'LIKE', "%{$search_column['meta_description']}%");
        }
        if ($search_column['status'] != '' && $search_column['status'] != NULL) {
            $sql->where('m_page.status', 'LIKE', "%{$search_column['status']}%");
        }
        if ($search_column['created_at'] != '' && $search_column['created_at'] != NULL) {
            $sql->where('m_page.created_at', 'LIKE', "%{$search_column['created_at']}%");
        }
        ...
        if ($search_column['updated_at'] != '' && $search_column['updated_at'] != NULL) {
            $sql->where('m_page.updated_at', 'LIKE', "%{$search_column['updated_at']}%");
        }
