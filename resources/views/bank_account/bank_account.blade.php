                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Bank Account
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('bank_accounts.store') : route('bank_accounts.update', base64_encode($data['bank_account']->id)) }}">
                                    @if($data['actions']=='update') @method('PUT') @endif
                                    @csrf
                                    <div class="form-group row">
                                        <label for="bank" class="col-sm-2 col-form-label">Bank</label>
                                        <div class="col-sm-10">
                                            <select class="form-control" id="bank" name="bank" required data-bind="value: id_m_bank,valueAllowUnset: true, options: $root.availableBank, 
                                            optionsText: 'name', optionsValue: 'id', select2: { placeholder: 'Choose Bank', 
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="account_name" name="account_name" placeholder="Enter account name" data-bind="value: account_name" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="account_number" class="col-sm-2 col-form-label">Account Number</label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="account_number" name="account_number" placeholder="Enter account number" data-bind="value: account_number" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
...
<script>
    ...
    function BankAccountViewModel() {
        ...
        self.account_name = ko.observable('<?php if (isset($data['bank_account'])) echo $data['bank_account']->account_name ?>');
        self.account_number = ko.observable('<?php if (isset($data['bank_account'])) echo $data['bank_account']->account_number ?>');
        self.id_m_bank = ko.observable('<?php if (isset($data['bank_account'])) echo $data['bank_account']->id_m_bank ?>');
        self.description = ko.observable('<?php if (isset($data['bank_account'])) echo $data['bank_account']->description ?>');
        self.status = ko.observable(<?= (isset($data['bank_account'])) ? (($data['bank_account']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new BankAccountViewModel());
</script>
@endsection