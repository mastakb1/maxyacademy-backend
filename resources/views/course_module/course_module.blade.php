                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Course Module
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('course_modules.store') : route('course_modules.update', base64_encode($data['course_module']->id)) }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" id="course" name="course" required data-bind="value: id_course,valueAllowUnset: true, options: $root.availableCourse, 
...
<script>
    ...
    function CourseModuleViewModel() {
        var self = this;

        self.availableCourse = ko.observableArray(<?php if (isset($data['course'])) echo $data['course'] ?>);

        self.name = ko.observable('<?php if (isset($data['course_module'])) echo $data['course_module']->name ?>');
        self.description = ko.observable(`<?php if (isset($data['course_module'])) echo $data['course_module']->description ?>`);
        self.id_course = ko.observable('<?php if (isset($data['course_module'])) echo $data['course_module']->id_course ?>');
        self.status = ko.observable(<?= (isset($data['course_module'])) ? (($data['course_module']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new CourseModuleViewModel());
</script>
@endsection