                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Difficulty Type
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('difficulty_types.store') : route('difficulty_types.update', base64_encode($data['difficulty_type']->id)) }}">
...
<script>
    ...
    function DifficultyTypeViewModel() {
        var self = this;

        self.name = ko.observable('<?php if (isset($data['difficulty_type'])) echo $data['difficulty_type']->name ?>');
        self.description = ko.observable('<?php if (isset($data['difficulty_type'])) echo $data['difficulty_type']->description ?>');
        self.status = ko.observable(<?= (isset($data['difficulty_type'])) ? (($data['difficulty_type']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new DifficultyTypeViewModel());
</script>
@endsection