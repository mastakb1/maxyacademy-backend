                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Course Type
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('course_types.store') : route('course_types.update', base64_encode($data['course_type']->id)) }}">
...
<script>
    ...
    function CourseTypeViewModel() {
        var self = this;

        self.name = ko.observable('<?php if (isset($data['course_type'])) echo $data['course_type']->name ?>');
        self.description = ko.observable('<?php if (isset($data['course_type'])) echo $data['course_type']->description ?>');
        self.status = ko.observable(<?= (isset($data['course_type'])) ? (($data['course_type']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new CourseTypeViewModel());
</script>
@endsection