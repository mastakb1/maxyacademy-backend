                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Modul
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('moduls.store') : route('moduls.update', base64_encode($data['modul']->id)) }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" id="course" name="course" required data-bind="value: id_m_course,valueAllowUnset: true, options: $root.availableCourse, 
...
<script>
    ...
    function ModulViewModel() {
        var self = this;

        self.availableCourse = ko.observableArray(<?php if (isset($data['course'])) echo $data['course'] ?>);

        self.name = ko.observable('<?php if (isset($data['modul'])) echo $data['modul']->name ?>');
        self.description = ko.observable('<?php if (isset($data['modul'])) echo $data['modul']->description ?>');
        self.id_m_course = ko.observable('<?php if (isset($data['modul'])) echo $data['modul']->id_m_course ?>');
        self.status = ko.observable(<?= (isset($data['modul'])) ? (($data['modul']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new ModulViewModel());
</script>
@endsection