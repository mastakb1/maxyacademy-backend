                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Course Batch
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('course_batches.store', $data['id_m_course']) : route('course_batches.update', ['id' => $data['id_m_course'], 'id_batch' => base64_encode($data['course_batch']->id)]) }}">
...
<script>
    ...
    function DiscountViewModel() {
        ...
        self.quota = ko.observable('<?php if (isset($data['course_batch'])) echo $data['course_batch']->quota ?>');
        self.status = ko.observable(<?= (isset($data['course_batch'])) ? (($data['course_batch']->status == 1) ? 'true' : 'false') : 'true' ?>);
        ...
        dateRange('date', self.date, '<?php if (isset($data['course_batch'])) echo $data['course_batch']->start_date ?>', '<?php if (isset($data['course_batch'])) echo $data['course_batch']->end_date ?>');
    }
