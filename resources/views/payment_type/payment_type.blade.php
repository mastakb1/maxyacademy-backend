                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Payment Type
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('payment_types.store') : route('payment_types.update', base64_encode($data['payment_type']->id)) }}">
...
<script>
    ...
    function PaymentTypeViewModel() {
        ...
        self.name = ko.observable('<?php if (isset($data['level'])) echo $data['level']->name ?>');
        self.description = ko.observable('<?php if (isset($data['level'])) echo $data['level']->description ?>');
        self.status = ko.observable(<?= (isset($data['level'])) ? (($data['level']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new PaymentTypeViewModel());
</script>
@endsection